...
class Home extends BaseController {
    protected function buildSelects() {  
        ...
        foreach ( $fields as $field => $modelClass ) {
            ...
            $cls = new \stdClass();
            $cls->fieldName = $field;
            $cls->options = $m->asIdValueMap();
            $cls->labelText = humanize($field);//)->select($field, $m->asIdValueMap());
            $result[$field] = $cls;
        }
        return $result;
    }
    ...
    public function index(): string {
        ...
        label($s->labelText)->select($s->fieldName, $s->options, [], '')->
        ...
        checkboxGroup('ghg', ['h', 'k', 'l'], [], 'class="inline"')->
