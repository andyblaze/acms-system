...
class FormBuilder {
    ...
    public function testStringToAttrArray(): void {
        $tests = [
            'Simple quoted'      => 'class="btn btn-primary"',
            'Unquoted value'     => 'id=saveBtn',
            'Boolean attribute'  => 'disabled',
            'Mixed attributes'   => 'class="btn btn-primary" id=saveBtn disabled',
            'Data attribute'     => 'data-info="some=thing"',
            'With single quotes' => "data-bs-target='#modal1'",
            'Spaces & mixed'     => 'placeholder="Enter your name here" required',
            'Nested quotes'      => 'data-bs-title="Click \'here\' to confirm"',
            'No quotes, multiple' => 'data-a=1 data-b=2 data-c=3',
            'Messy spacing'      => '   class = "btn btn-warning"    id = btn42   ',
        ];

        echo "<pre>🔍 Testing stringToAttrArray()\n\n";

        foreach ($tests as $label => $input) {
            $result = $this->stringToAttrArray($input);
            echo "🧪 $label:\n";
            echo "   Input : $input\n";
	...
	protected function sanitise_string($str) {
    ...
    protected function stringToAttrArray(string $attrString): array {
        $attrs = [];
        $str = $this->sanitise_string($attrString);  echo $str ."<br>";
        $parts = preg_split('/\s/', trim($str));

        foreach ($parts as $part) {
            if ( str_contains($part, '=') ) {
                [$key, $value] = explode('=', $part, 2);
                $value = trim($value, "\"'"); // remove " or '
                $attrs[$key] = $value;
            } elseif ($part !== '') {
    ...
    protected function mergeAttributes(string $type, $extra): array {
        $defaults = $this->bootstrapConfig[$type] ?? [];

        // Normalize $extra into an array
        if ( ! is_array($extra) ) {
            // Parse 'key="value"' strings into an array if needed
            $extra = $extra ? $this->stringToAttrArray($extra) : [];
        }

        // Merge classes properly (append instead of overwrite)
        if ( isset($defaults['class']) && isset($extra['class']) ) {
            $extra['class'] = trim($defaults['class'] . ' ' . $extra['class']);
